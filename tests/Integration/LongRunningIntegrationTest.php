...
final class LongRunningIntegrationTest extends TestCase
{
    ...
    public function testThousandRequestsWithResetAndMemoryStability(): void
    {
        ...
        for ($i = 1; $i <= self::REQUEST_COUNT; $i++) {
            ...
            $this->assertSame(200, $response->statusCode, "Request $i: expected 200");
            $this->assertSame('OK', $response->endContent, "Request $i: expected body 'OK'");
        ...
        $this->assertSame(self::REQUEST_COUNT, $kernel->handleCount);
        $this->assertSame(self::REQUEST_COUNT, $kernel->terminateCount);
        $this->assertSame(self::REQUEST_COUNT, $kernel->resetCount);
        $this->assertSame(self::REQUEST_COUNT, $resetHook->resetCount);

        // Memory stability: RSS at req 1000 ≤ 2× RSS at req 10
        $this->assertNotNull($memoryAtReq10);
        $this->assertNotNull($memoryAtReq1000);
        ...
        $this->assertSame(self::REQUEST_COUNT, $snapshot['symfony_requests_total']);
    }
    ...
    public function testRequestCounterIncrements(): void
    {
        ...
        $this->assertSame(50, $adapter->getRequestCount());
    }
}
