...
final class StreamingIntegrationTest extends TestCase
{
    public function testStreamedResponseWithLargeVolume(): void
    {
        ...
        $streamedResponse = new StreamedResponse(static function () use ($chunkCount, $chunkData): void {
        ...
        self::assertNotEmpty($response->writes, 'StreamedResponse should produce write() calls');

        // Total written content should equal chunkCount * chunkData
        $totalWritten = implode('', $response->writes);
        self::assertSame($chunkCount * mb_strlen($chunkData), mb_strlen($totalWritten));

        // end('') called after streaming
        self::assertTrue($response->endCalled);
        self::assertSame('', $response->endContent);
    }
    ...
    public function testSseResponseWithImmediateFlush(): void
    {
        ...
        $sseResponse = new StreamedResponse(static function () use ($events): void {
        ...
        self::assertArrayHasKey('X-Accel-Buffering', $response->headers);
        ...
        foreach ($events as $event) {
            self::assertStringContainsString(mb_trim($event), $allWritten);
        }
    }
